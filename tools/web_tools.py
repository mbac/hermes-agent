...
def _get_backend_candidates(capability: str | None = None) -> list[str]:
    ...
    When *capability* is ``"search"``, SearXNG is included in the priority
    list (it is search-only).  For ``"extract"`` and ``"crawl"`` it is
    excluded.  Per-capability config (``web.{capability}_backend``) takes
    precedence over the shared ``web.backend`` setting.
    """
    if capability == "search":
        configured = (_get_search_backend() or "").lower().strip()
        PRIORITY = ("firecrawl", "parallel", "tavily", "exa", "searxng")
    ...
    if configured in PRIORITY and CANDIDATE_CHECKS[configured]():
...
def _try_backend_with_fallback(
    ...
    """
    candidates = _get_backend_candidates(operation)
...
def _searxng_search(query: str, limit: int) -> dict:
    """Search using SearXNG and return results as a standard dict."""
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        def _brave_free_search(q: str, lim: int) -> dict:
            from tools.web_providers.brave_free import BraveFreeSearchProvider
            return BraveFreeSearchProvider().search(q, lim)

        def _ddgs_search(q: str, lim: int) -> dict:
            from tools.web_providers.ddgs import DDGSSearchProvider
            return DDGSSearchProvider().search(q, lim)
        ...
        fn_map = {
            ...
            "searxng":    lambda: _searxng_search(query, limit),
            "brave-free": lambda: _brave_free_search(query, limit),
            "ddgs":       lambda: _ddgs_search(query, limit),
        }
...
async def web_extract_tool(
    ...
    try:
        ...
        # Search-only backends — refuse extract before any URL processing
        _eb = _get_extract_backend()
        if _eb in ("searxng", "brave-free", "ddgs"):
            _label = {"searxng": "SearXNG", "brave-free": "Brave Search (free tier)", "ddgs": "DuckDuckGo (ddgs)"}[_eb]
            return json.dumps({
                "success": False,
