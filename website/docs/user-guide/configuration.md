...
**Runtime failover:** When a configured backend fails at runtime (timeout, 5xx, rate-limit, quota exhaustion), Hermes automatically retries the next available backend in priority order. Setting `web.backend` (or the per-capability `web.search_backend` / `web.extract_backend`) places that backend first in the chain but does not disable failover — other backends with valid API keys are still tried on transient failures. Errors that no backend swap can fix (400 bad request, 401/403 auth errors, 404 not found, 422 invalid parameters, malformed input, blocked URLs) short-circuit the chain immediately to avoid wasting API quota on the same bad request. Rate-limit errors (429) remain retryable since a different provider may have available quota.

**Fallback on empty extract** (opt-in): set `web.fallback_on_empty_extract: true` to advance to the next extract backend when the current one returned without raising but produced no usable content for any non-blocked URL. Useful when a provider intermittently soft-fails (200 OK with empty body, anti-bot redirects, JS-only pages, paywalls). Policy-blocked and SSRF-blocked rows are excluded from the empty check — those URLs are intentionally refused and would be refused by any backend. Defaults to `false`, since for some workflows an empty extract is a legitimate signal that a page genuinely has no extractable text and a second billed call is undesirable.

**Strict backend pin** (opt-in): set `web.strict_backend: true` to disable cross-provider failover and honor explicit backend pins absolutely. With this flag on, only the configured backend (`web.backend`, or `web.search_backend` / `web.extract_backend` when set) is tried, and runtime failures surface directly without silent provider switching. Useful for cost control, compliance, or debugging. Defaults to `false`, preserving the soft-preference behaviour for existing configs.
